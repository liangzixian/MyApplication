...
import com.mrliang.androidlib.utils.BaseUtils;
import com.mrliang.androidlib.utils.FrameConstants;
        ...
        try {
            ...
            if ((requestCallback != null)) {
                if (statusCode == HttpStatus.SC_OK) {
                    ...
                    if (responseInJson.getError_code() != 0) {
                        handleNetworkError(responseInJson.getReason());
                    } else {
                        // 把成功获取到的数据记录到缓存
                        if (urlData.getNetType().equals(REQUEST_GET) && urlData.getExpires() > 0) {
                            CacheManager.getInstance().putFileCache(newUrl, responseInJson.getResult(),
                                    urlData.getExpires());
                        }
            ...
            if (header != null) {
                ...
                try {
                    if ((strServerDate != null) && !strServerDate.equals("")) {
                        ...
                        deltaBetweenServerAndClientTime = serverDateUAT.getTime() + 8 * 60 * 60 * 1000
